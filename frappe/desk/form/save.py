...
def savedocs(doc, action):
	"""save / submit / update doclist publish rejected"""
	try:
...
@frappe.whitelist()
def reject(doctype=None, name=None):
	"""Reject the rfq status"""
	try:
		doc = frappe.get_doc(doctype, name)
		doc.reject()
		send_updated_docs(doc)
		# frappe.msgprint(frappe._("Rejected"), indicator='red', alert=True)
		return 1
	except Exception:
		frappe.errprint(frappe.utils.get_traceback())
		raise
