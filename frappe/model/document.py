...
from frappe.model.naming import set_new_name, gen_new_name_for_cancelled_doc,gen_new_name_for_rejected_doc
...
from tradegully.custom_api.rfq.rfq_util import admin_submit
...
### rejected method for rfq
	@whitelist.__func__
	def _reject(self):
		"""cancel the doc and set docstate = 2"""
		state = self.docstatus
		try:
			frappe.db.sql(f'''call set_rejected('{self.name}',@err)''')
			rf = frappe.db.sql('''select @err as error''',as_dict=1)
			# print(f'ERROROROROROR:::{rf[0].get("error")}')
			if rf[0].get('error') == 0:
				# new_name = gen_new_name_for_rejected_doc(self)
				# frappe.rename_doc(self.doctype, self.name, new_name, force=True, show_alert=False)
				# self.name = new_name
				# self.save()
				return 1
	...
	def submit(self):
		...
		# 	rfq_id = self.name
		# 	items = frappe.db.sql(
		# 		f''' select item_code, item_group from `tabRequest for Quotation Item` where parent = '{rfq_id}' limit 1''',
		# 		as_dict=1)
		# 	if not items:
		# 		return "No Supplier"
		# 	item_code = items[0].get('item_code')
		# 	item_group = items[0].get('item_group')
		# 	if not item_group:
		# 		item_grp = frappe.db.sql(f''' select item_group from `tabItem` where name = '{item_code}' ''',
		# 								 as_dict=1)
		# 		item_group = item_grp[0].get('item_group')
		#
		# 	if not item_group:
		# 		return "No Item Group"
		# 	item_group_parent = frappe.db.sql(
		# 		f''' select t2.parent from `tabItem Group` t1 join `tabItem Group` t2 on t1.parent = t2.name where t1.name like '%{item_group}%' ''',
		# 		as_dict=1)
		# 	itm_grp_parent = item_group_parent[0].get('parent')
		#
		# 	suppliers = frappe.db.sql(
		# 		f''' select name from `tabSupplier` where supplier_group like '%{itm_grp_parent}%' ''',
		# 		as_dict=1)
		#
		# 	if len(suppliers) > 0:
		# 		self.update_children()
		...
		# 		for y in suppliers:
		# 			a = y.get('name')
		# 			s = frappe.get_doc('Supplier', a)
		# 			rfq.append('suppliers', {
		# 				'supplier': s.name,
		# 				'send_email': 0,
		# 			})
		# 		frappe.db.set_value('Request for Quotation', rfq.name, 'workflow_state', 'Submitted')
		# 		# rfq.save()
		# 		# global count
		# 		# count = True
		# 		self.save()
